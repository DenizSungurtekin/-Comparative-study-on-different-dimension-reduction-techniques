...
## Personal implementation of UMAP

def computeProbMatrix(sigmas,dist,rhos): #Compute entire pi|j matrix, sigmas 1d vector containing sgd for each point
    N = len(dist)
    prob_matrix = np.asarray([[computeProba(dist[i][j],sigmas[i],rhos[i]) if i!=j else 1 for j in range(N)] for i in range(N)])
    return prob_matrix

def computeProba(dist,sigma,rho): # Given a distance sigma and rho (nearest neighbor distance) compute pi|j
    return np.exp(-((dist-rho)/sigma))

def p_conditional_to_joint(P):
    return (P + P.T) - np.multiply(P,P.T)

def compute_k(P_row): #return k for a row c.f paper
    return 2**P_row.sum()

def computeProbEmbedMatrix(Y,a,b): # Compute low dimensional probability matrix
    prob_matrix = np.power(1 + a * np.square(euclidean_distances(Y, Y))**(2*b),-1)
    return prob_matrix

def computeGrad(P,Y,a,b): # Compute gradient
...
def compute_p_row(dist,sigma,rho,normalization): # Compute probability pi|j by row used in binary search
    if normalization:
...
def find_best_a_b(min_dist): # a and b determine the strength of membership function. (c.f paper)
...
def umap_reduction(data,min_dist,target_dim,target_k,normalization = False,epochs = 200,lr = 0.02,initialization = "random", mu = 0, std = 1,SGD = False): # Perform reduction
...
def cross_entropy(P,Y,a,b): # Compute cross entropy for entier matrix
